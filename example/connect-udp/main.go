...
func main() {
	...
	for _, addr := range urls {
		...
		go func(addr string) {
			fullAddr := fmt.Sprintf("masque://%s", addr)
			...
			logger.Infof("Got connection object: %+v", conn)

			b := []byte{}
			b = quicvarint.Append(b, uint64(flowID))
			b = append(b, []byte("hellow world from quic-go")...)
			...
			logger.Infof("Sent Datagram: %+v", b)

			recv, err := conn.ReceiveDatagram(context.Background())
			if err != nil && err != io.EOF {
				log.Fatal("Failed to receive datagram: %v", err)
			}
